...
test_g_mutex (void)
{
  ...
  g_mutex_unlock (test_g_mutex_mutex);
  cut_assert_equal_int (41, GPOINTER_TO_INT (g_thread_join (thread)));
...
test_g_static_rec_mutex_thread (gpointer user_data)
{
  ...
  g_static_rec_mutex_lock (&test_g_static_rec_mutex_mutex);
  cut_assert_equal_int (42, test_g_static_rec_mutex_int);
      ...
      else /* I'm a writer */
	{
	  ...
	  if (g_random_double() > .2) /* I'll block */ 
	    ...
	    if (!g_static_rw_lock_writer_trylock (&test_g_static_rw_lock_lock))
	      continue;
	  G_LOCK (test_g_static_rw_lock_state);
	  cut_assert_equal_int (0, test_g_static_rw_lock_state);
...
test_g_thread_once (void)
{
  ...
  for (i = 0; i < G_ONCE_SIZE; i++) 
    {
      cut_assert_equal_int (i + 1, test_g_once_guint_array[i]);
    }
}
