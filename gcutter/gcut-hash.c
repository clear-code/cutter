...
static void
equal_hash_pair (gpointer key, gpointer value, gpointer user_data)
{
...
gboolean
gcut_hash_equal (GHashTable *hash1, GHashTable *hash2,
                 GCompareDataFunc compare_func, gpointer user_data)
{
    ...
    data.equal = TRUE;
    g_hash_table_foreach(hash1, equal_hash_pair, &data);
...
static void
inspect_hash_pair (gpointer key, gpointer value, gpointer user_data)
{
...
gchar *
gcut_hash_inspect (GHashTable *hash,
                   GCutInspectFunc key_inspect_func,
                   GCutInspectFunc value_inspect_func,
                   gpointer user_data)
{
    ...
    g_hash_table_foreach(hash, inspect_hash_pair, &data);
...
gboolean
gcut_hash_string_equal (GHashTable *hash1, GHashTable *hash2)
{
    return gcut_hash_equal(hash1, hash2, equal_string, NULL);
}
...
gchar *
gcut_hash_string_string_inspect (GHashTable *hash)
{
    return gcut_hash_inspect(hash, inspect_string, inspect_string, NULL);
}
